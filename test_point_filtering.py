...
from pytorch3d.renderer import cameras as ptcamera
...
class AstyxDataset(Dataset):
        ...
        for file in os.listdir(self.radar_dir):
            ...
            if idx not in self.ids:
                ...
                if not os.path.exists(self.calibration_dir + f"/{idx:06d}.json"):
                    print("File not found: ", self.calibration_dir + f"/{idx:06d}.json")
                    continue
                if not os.path.exists(self.camera_dir + f"/{idx:06d}.jpg"):
                    print("File not found: ", self.camera_dir + f"/{idx:06d}.jpg")
                    continue
                if not os.path.exists(self.object_dir + f"/{idx:06d}.json"):
                    print("File not found: ", self.object_dir + f"/{idx:06d}.json")
                    continue
                self.ids.append(idx)
    ...
    def __getitem__(self, idx):
        ...
        # depth = depth[:, square_image_offset : square_image_offset + depth.shape[0]]
        # load radar data, .txt
        df = pd.read_csv(
        ...
        while radar_data.shape[0] < self.N:
            radar_data = torch.cat(
                [
                    radar_data,
                    radar_data[
                        torch.randperm(radar_data.size(0))[
                            : self.N - radar_data.shape[0]
                        ]
                    ],
                ]
            )
        ...
        with open(self.calibration_dir + f"/{idx:06d}.json") as f:
            ...
            for c in calibrations["sensors"]:
                if c["sensor_uid"] == "camera_front":
                    ...
                    new_principal_point = camera_base.principal_point  # - torch.tensor(
                    # [square_image_offset, 0], device=camera_base.device
                    # )

                    new_camera_base = PerspectiveCameras(
                        focal_length=camera_base.focal_length.float(),
                        principal_point=new_principal_point.float(),
                        R=camera_base.R.float(),
                        T=camera_base.T.float(),
                    )
        assert (
        ...
        points_uv, _ = points_to_uv(radar_data, new_camera_base)
...
def custom_collate_fn(batch):
    depths, radar_data, camera_base_list, camera_fronts, objects, idxs, npoints = zip(
        *batch
    )
...
def train_one_epoch(
    ...
    for batch in dataloader:
        ...
        depths, radar_data, camera, image_rgb, objects, idx, npoints = batch
...
def plot_image_depth_projected(
    ...
    if camera is not None:
        ...
        cam_wires_trans = get_camera_wires_trans(camera, scale=10).detach().cpu()
        assert cam_wires_trans.device == torch.device(
            "cpu"
        ), "cam_wires_trans should be on cpu"
    assert gt.device == torch.device("cpu"), "gt should be on cpu"
    ...
    fig = plt.figure(figsize=(40, 10 * len(gt)))

    losses = []
    for i in range(len(gt)):
        # print("device", camera[i].device)
        projected_points, points_camera = points_to_uv(
            torch.Tensor(gt[i]).to(camera[i].device), camera[i]
        )
        projected_points = projected_points.cpu().numpy()
        # projected_points = (
        #     points_to_uv(torch.Tensor(gt[i]).to(camera[i].device), camera[i])
        #     .cpu()
        #     .numpy()
        # )
        # print("projected_points", projected_points.shape)  # projected_points (2028, 2)
        # # print min max for each axis
        # print("min", np.min(projected_points, axis=0))
        # print("max", np.max(projected_points, axis=0))

        ax = fig.add_subplot(len(gt), 4, 4 * i + 1)
        ax.imshow(image_rgb[i].cpu().numpy().transpose(1, 2, 0))
        ax.scatter(
            projected_points[:, 0],
            projected_points[:, 1],
            ...
            s=point_size,
            # c=gt[i][:, 0],
            c=points_camera[:, 2].cpu().numpy(),
            cmap="jet",
        )
        ax.axis("off")
        ax.set_title("image_rgb")
        # setlims to image area only
        ax.set_xlim(0, image_rgb[i].shape[2])
        ax.set_ylim(image_rgb[i].shape[1], 0)
        if depth_image is not None:
            ax = fig.add_subplot(len(gt), 4, 4 * i + 2)
            # show gray scale image, take only the first layer
            #  depth_image torch.Size([618, 2048])
            # print("depth_image", depth_image[i].shape)
            plot = ax.imshow(depth_image[i].cpu().numpy())
            ...
            max_z_point_camera = points_camera[:, 2].max()
            min_z_point_camera = points_camera[:, 2].min()
            # print("max_z_point_camera", max_z_point_camera)
            # print("min_z_point_camera", min_z_point_camera)
            # normalize z from 0 - 1

            # print("max_z_depth", max_z_depth)
            # print("min_z_depth", min_z_depth)

            x = list(range(depth_image[i].shape[0]))
            y = list(range(depth_image[i].shape[1]))
            fun = interpolate.interp2d(y, x, depth_image[i].cpu().numpy())
            depth_depth = []
            radar_depth = []
            for j in range(len(projected_points)):
                # if point in image
                if (
                    0 <= projected_points[j][0] < depth_image[i].shape[1]
                    and 0 <= projected_points[j][1] < depth_image[i].shape[0]
                ):
                    # get the depth from the pixel value using interpolate.interp2d
                    depth_new = fun(projected_points[j][0], projected_points[j][1])

                    # print("depth_new", depth_new )
                    # print("points_camera[j,2]",j,points_camera.shape)
                    # print("depth_new",depth_new)
                    # print("points_camera",points_camera)
                    # print("type",type(depth_new),type(normalized_z_cam[j]))
                    losses.append((depth_new[0] - points_camera[j, 2]) ** 2)
                    depth_depth.append(depth_new[0])
                    radar_depth.append(points_camera[j, 2].cpu().numpy())

                # else:

                #     print("not in",projected_points[j], depth_image[i].shape)
            ...
            # setlims to depth area only
            ax.set_xlim(0, image_rgb[i].shape[2])
            ax.set_ylim(image_rgb[i].shape[1], 0)

            # end EXP

            ax = fig.add_subplot(len(gt), 4, 4 * i + 3)
            plot = ax.scatter(
                depth_depth,
                radar_depth,
                marker="x",
            ...
            ax.set_xlabel("depth image")
            ax.set_ylabel("radar depth")

        ax = fig.add_subplot(len(gt), 4, 4 * i + 4, projection="3d")
        # color by z
        ...
        if cam_wires_trans is not None:
            ...
            # print("coord", x_, y_, z_)
            (h,) = ax.plot(x_, y_, z_)
            # rotate the view to follow the camera
            R = camera[i].R.cpu().numpy()[0]
            # print("R", R)
            # print("shape", R.shape)
            # calculate azimuth from R, 3d rotation matrix
    ...
    plt.close()
    # print("saved image-mask", fname)
    print("losses", len(losses), "factor", factor, sum(losses) / len(losses))
...
def save_checkpoint(
    model, optimizer, cfg, cd_list, loss_list, checkpoint_fname, db_fname  # (epoch, cd)
):
        ...
        for epoch, losses in enumerate(prev_losses):
            ...
            for alpha in loss_ema_factors:
                if loss_emas[alpha] is None:
                    ...
                    loss_emas[alpha] = (1 - alpha) * losses + alpha * loss_emas[alpha]
        ...
        for epoch, cd_loss in prev_cds:
            ...
            for alpha in cd_emas:
                if cd_emas[alpha] is None:
                    ...
                    cd_emas[alpha] = (1 - alpha) * cd_loss + alpha * cd_emas[alpha]
        ...
        for epoch in tqdm_range:
            ...
            if (epoch + 1) % cfg.run.vis_freq == 0:
                ...
                for alpha in cd_emas:
                    ...
                    # )
                    writer.add_scalars(
                        "CD_condition", {f"ema/{alpha:.4e}": cd_emas[alpha]}, epoch
                    )
                plot_sample_condition(
                    pc_condition.cpu(),
                    xts.cpu(),
...
def points_to_uv(points, cameras):
    focal_length, principal_point, R, T = (
    ...
    """
    Convert 3D points to 2D image coordinates (u, v).

    Args:
        points (torch.Tensor): 3D points of shape (N, 3).
        focal_length (torch.Tensor): Focal length of shape (1, 2).
        principal_point (torch.Tensor): Principal point of shape (1, 2).
        R (torch.Tensor): Rotation matrix of shape (3, 3).
        T (torch.Tensor): Translation vector of shape (3,).

    Returns:
        torch.Tensor: Projected 2D points of shape (N, 2).
    """
    # Transform points to the camera coordinate system
    # print("-----------------------------------------")
    # print("shapes", points.shape, R.shape, T.shape)
    # print("types", type(points), type(R), type(T))
    # print("R", R)
    # print("points", points[0])
    points_camera = torch.mm(R.float().T, points.transpose(0, 1).float()).transpose(
        0, 1
    )
    # print("points_camera", points_camera[0])
    # print("T", T)
    points_camera += T
    # print("points_camera", points_camera[0])
    ...
    u = focal_length[0, 0] * X_camera / Z_camera + principal_point[0, 0]
    v = focal_length[0, 1] * Y_camera / Z_camera + principal_point[0, 1]

    points_image = torch.stack([u, v], dim=1)

    # print("points_image", points_image.shape)
    # print("-----2D points (camera coordinates):", points_image[0])
    return points_image, points_camera

def points_to_uv3(points, cameras):
    focal_length, principal_point, R, T = (
        cameras.focal_length,
        cameras.principal_point,
        cameras.R.squeeze(0),
...
def points_to_uv2(points, cameras):
    
    assert points.device == cameras.device, "points and cameras must be on the same device"
    # points (torch.Tensor): 3D points of shape (N, 3).
    focal_length, principal_point, R, T = (
        cameras.focal_length,
        cameras.principal_point,
    ...
    # print("RT", RT.shape)#RT torch.Size([3, 4])
    #RT must be 4 x 4
    RT = torch.cat([RT, torch.tensor([[0, 0, 0, 1]]).to(RT.device)], dim=0)
    # print("RT", RT.shape)

    K = cameras.get_projection_transform().get_matrix()[0, :3, :3].to(RT.device)
    # print("K", K.shape)
    # print("K", K)

    myK = torch.tensor([[focal_length[0, 0], 0, principal_point[0, 0]], [0, focal_length[0, 1], principal_point[0, 1]], [0, 0, 1]]).to(RT.device)
    # print("myK", myK.shape)
    # print("myK", myK)

    # Based on the naming "T_to_ref_COS", it is most likely a forward transformation (mapping from "T" to "ref_COS"
    ...
    # Transform points to the camera coordinate system
    cam_coord = torch.mm(RT.T, points.transpose(0, 1))  # 4 x N
    
    cam_coord = cam_coord / cam_coord[3, :]

    assert cam_coord[3, :].allclose(torch.ones(cam_coord.shape[1]).to(cam_coord.device) ), f"last row of cam_coord must be 1 {cam_coord[3, :5]}"
    # print("cam_coord", cam_coord.shape)
    # print("cam_coord", cam_coord[:,:5].T)

    image_coord = torch.mm(myK, cam_coord[:3, :])  # 3 x N

    image_coord = image_coord / image_coord[2, :]

    
    assert image_coord[2, :].allclose(torch.ones(image_coord.shape[1]).to(image_coord.device) ), f"last row of image_coord must be 1 {image_coord[2, :5]}"

    # print("image_coord", image_coord.shape)
    # print("image_coord", image_coord[:,:5].T)
    
    return image_coord.T[:,:2] ,cam_coord.T[:,:3] 


def points_to_uv_pt3d(points, cameras):

    points_camera = cameras.transform_points(points).squeeze(0)
    # divide by last column
    # points_camera = points_camera[:, :2] / points_camera[:, 2:]
    return points_camera


# def points_to_uv_notwork(points, cameras):
#     """
#     Filter points that are visible in the image.

#     Args:
#         points (torch.Tensor): 3D points of shape (N, 3).
#         cameras (PerspectiveCameras): PerspectiveCameras object.

#     Returns:
#         points_image (torch.Tensor): Projected 2D points of shape (N, 2).
#     """
#     # Transform points to the camera coordinate system
...
#     #divide by last column
#     points_camera = points_camera[:, :2] / points_camera[:, 2:]
#     # print("Transformed points (camera coordinates):", points_camera)
#     points_image = points_camera[:, :2]
#     print("points_image", points_image.shape)
#     print("-----2D points (camera coordinates):", points_image[0])
#     print("min, max", points_image.min(dim=0), points_image.max(dim=0))

#     #calculates manually

#     T = cameras.T
#     T2 = cameras.get_camera_center()
#     print("T vs T2", T, T2)
#     R = cameras.R
#     print("R", R)
...
#     # focal_length = torch.tensor([[K[0, 0], K[1, 1]]])
#     # principal_point = torch.tensor([[K[0, 2], K[1, 2]]])
#     print("focal_length", cameras.focal_length)
#     print("principal_point", cameras.principal_point)
#     K = cameras.get_projection_transform().get_matrix()[0, :3, :3]
#     print("K", K)

#     points = points.squeeze(0)
#     print("shape points", points.shape)
#     print("np.transpose(points) points", points.transpose(0,1).shape)
#     print("shape R", R.shape)
#     # shape points torch.Size([1, 2000, 3])
#     # np.transpose(points) points torch.Size([3, 2000])
#     # shape R torch.Size([1, 3, 3])
#     R = R.squeeze(0)
#     print("shape R", R.shape, type(R))
#     print("type points", type(points))
#     obj_camera = torch.mm(R, points.transpose(0, 1))
#     print("obj_camera", obj_camera.shape)

#     print("T", T.shape)
#     print("T[:, np.newaxis]", T[:, np.newaxis].shape)
#     # T torch.Size([1, 3])
#     # T[:, np.newaxis] torch.Size([1, 1, 3])
#     T = T.squeeze(0)
#     print("T", T.shape)
#     print("T[:, np.newaxis]", T[:, np.newaxis].shape)
#     # T torch.Size([3])
#     # T[:, np.newaxis] torch.Size([3, 1])
#     print("types", type(obj_camera), type(T[:, np.newaxis]))

#     # types <class 'numpy.ndarray'> <class 'torch.Tensor'>
#     obj_camera = torch.tensor(obj_camera)


#     obj_camera = obj_camera + T[:, np.newaxis]
#     print("types K, obj_camera", type(K), type(obj_camera))
#     # types K, obj_camera <class 'torch.Tensor'> <class 'torch.Tensor'>
#     print("shape K", K.shape)
#     # shape K torch.Size([3, 3])
#     obj_image = torch.mm(K, obj_camera)
#     obj_image = obj_image / obj_image[2, :]

#     obj_image = obj_image.T
#     obj_image = obj_image[:, :2]
#     print("shape obj_image", obj_image.shape)
#     print("-----obj_image", obj_image[0])
#     print("min, max", obj_image.min(dim=0), obj_image.max(dim=0))
...
# def filter_points_in_image(points, cameras, image_width, image_height):
#     """
#     Filter points that are visible in the image.
...
#     Returns:
#         torch.Tensor: Filtered points that are visible in the image.
#     """
#     # Convert points to homogeneous coordinates
#     ones = torch.ones((points.shape[0], 1), device=points.device)
#     points_homogeneous = torch.cat([points, ones], dim=1)

#     # Get the camera extrinsic matrix (world to camera transformation)
#     R = cameras.R[0]
#     T = cameras.T[0]
#     extrinsic_matrix = torch.eye(4, device=points.device)
#     extrinsic_matrix[:3, :3] = R
#     extrinsic_matrix[:3, 3] = T

#     # Transform points to the camera coordinate system
#     points_camera = points_homogeneous @ extrinsic_matrix.T

#     # Get the camera intrinsic matrix
#     K = cameras.get_projection_transform().get_matrix()[0, :3, :3]

#     # Project points onto the image plane using the intrinsic matrix
#     points_image = points_camera[:, :3] @ K.T
...
#     mask = (
#         (points_image[:, 0] >= 0)
#         & (points_image[:, 0] < image_width)
#         & (points_image[:, 1] >= 0)
#         & (points_image[:, 1] < image_height)
#         & (points_camera[:, 2] > 0)  # Ensure points are in front of the camera
#     )

#     return points[mask], points_image[mask]


# def extract_astyx_batch(
#     batch, cfg: ProjectConfig, device, square_image_offset=int((2048 - 618) / 2)
# ):
...
#     assert (
#         0 <= square_image_offset <= 2048 - 618
#     ), "square_image_offset must be between 0 and 2048-618"
...
#         [square_image_offset, 0], device=camera_bases.device
#     )
#     new_camera_bases = PerspectiveCameras(
#         focal_length=camera_bases.focal_length,
...
#     # filter points
#     points = radar_data.float().to(device)
#     # image_width, image_height  = cfg.dataset.image_size, cfg.dataset.image_size
#     # filter_points_in_image(points, cameras, image_width, image_height)

#     pc = Pointclouds(points=points)
#     # pc, camera, image_rgb, mask
#     return (
#         pc,
#         new_camera_bases.to(device),
#         new_camera_rgb.to(device),
#         None,
#         new_depths.to(device),
#     )
...
def euler_from_pytorch3d(R, convention):
    from pytorch3d.transforms import matrix_to_euler_angles

    return matrix_to_euler_angles(R, convention)


def euler_from_scipy(R, convention):
    from scipy.spatial.transform import Rotation

    ### first transform the matrix to euler angles
    r = Rotation.from_matrix(R)
    return r.as_euler(convention.lower(), degrees=False)


def euler_from_manual(R, convention):

    assert convention == "ZXZ", "only support ZXZ convention"
    alpha = torch.atan2(R[2, 0], R[2, 1])
    beta = torch.acos(R[2, 2])
    gamma = torch.atan2(R[0, 2], R[1, 2])
    return torch.tensor([alpha, beta, gamma])


import matplotlib.colors as mcolors
def save_bulk_projected_image( fname, point_data, point_size=1,  image_rgb=None,title="",limit_view=False):

    fig = plt.figure(figsize=(16,5 ))
    ax = fig.add_subplot(111)
    markers = [".", "x", "o", "v", "^", "<", ">", "1", "2", "3", "4", "8", "s", "p", "P", "*", "h", "H", "+", "x", "X", "D", "d", "|", "_"]

    ax.imshow(image_rgb.cpu().numpy().transpose(1, 2, 0))
    for i,(name,projected_points,points_camera,color) in enumerate(point_data):
        projected_points = projected_points.cpu().numpy()
        if color is None:
            cc=  [mcolors.BASE_COLORS[k] for k in mcolors.BASE_COLORS]
            print("cc", cc)
            color = list([cc[i]])* len(projected_points)
...
def save_projected_image(gt, fname, point_size=1, camera=None, image_rgb=None, i=0,title=""):
    ...
    if title == "method1":
        ...
        projected_points, points_camera = points_to_uv2(
            torch.Tensor(gt[i]).to(camera[i].device), camera[i]
        )
    elif title == "method3":
        projected_points, points_camera = points_to_uv3(
            torch.Tensor(gt[i]).to(camera[i].device), camera[i]
        )
    projected_points = projected_points.cpu().numpy()

    print("---points_camera", points_camera[:5,:])
    print("---projected_points", projected_points[:5,:])
...
def main(cfg: ProjectConfig):
    ...
    if False:
        # best one is to transform_points
        angles_in = torch.tensor([90, 0, 0]) / 180 * 3.14159
        tx_in = torch.tensor([0.0, 0.0, 0.0])

        RT = torch.tensor(
            [
                [
                    torch.cos(angles_in[1]) * torch.cos(angles_in[2]),
                    -torch.cos(angles_in[0]) * torch.sin(angles_in[2])
                    + torch.sin(angles_in[0])
                    * torch.sin(angles_in[1])
                    * torch.cos(angles_in[2]),
                    torch.sin(angles_in[0]) * torch.sin(angles_in[2])
                    + torch.cos(angles_in[0])
                    * torch.sin(angles_in[1])
                    * torch.cos(angles_in[2]),
                    tx_in[0],
                ],
                [
                    torch.cos(angles_in[1]) * torch.sin(angles_in[2]),
                    torch.cos(angles_in[0]) * torch.cos(angles_in[2])
                    + torch.sin(angles_in[0])
                    * torch.sin(angles_in[1])
                    * torch.sin(angles_in[2]),
                    -torch.sin(angles_in[0]) * torch.cos(angles_in[2])
                    + torch.cos(angles_in[0])
                    * torch.sin(angles_in[1])
                    * torch.sin(angles_in[2]),
                    tx_in[1],
                ...
                [
                    -torch.sin(angles_in[1]),
                    torch.sin(angles_in[0]) * torch.cos(angles_in[1]),
                    torch.cos(angles_in[0]) * torch.cos(angles_in[1]),
                    tx_in[2],
                ],
                [0.0, 0.0, 0.0, 1.0],
            ]
        ...
        # find camera Euler angles (z-x-z extrinsic rotation)
        # alpha = torch.atan2(RT[2,0], RT[2,1])
        # beta = torch.acos( RT[2, 2]
        # )
        # gamma = torch.atan2(RT[0,2], RT[1,2])

        euler_angles_0 = euler_from_manual(RT[:3, :3], "ZXZ")
        alpha = euler_angles_0[0]
        beta = euler_angles_0[1]
        gamma = euler_angles_0[2]

        print(
            "alpha, beta, gamma  (z-x-z)",
            alpha / 3.14159 * 180,
            beta / 3.14159 * 180,
            gamma / 3.14159 * 180,
        )
        ...
        camera = PerspectiveCameras(
            focal_length=torch.tensor([[1817.98103, 1816.83987]]),
            principal_point=torch.tensor([[1040.27484, 319.497539]]),
            R=RT[:3, :3].unsqueeze(0),
            T=RT[:3, 3].unsqueeze(0),
        )
        print("camera R", camera.R)
        print("camera T", camera.T)
        euler_angles = euler_from_pytorch3d(RT[:3, :3], "ZXZ")

        euler_angles2 = euler_from_scipy(RT[:3, :3], "ZXZ")

        print("euler_angles torch", euler_angles / 3.14159 * 180)
        print("euler_angles scipy", euler_angles2 / 3.14159 * 180)

        # point =  torch.tensor([24.8348, -6.0178,  1.0476])
        point = torch.tensor([[1, 0, 0], [1, 0, 1]])
        point = point.transpose(0, 1)

        print("shape`", point.shape)

        print("shape", torch.ones(1, point.shape[0]).shape)
        # add 1 to the end of each point
        point = torch.cat([point, torch.ones(1, point.shape[1])], dim=0)
        print("point", point, point.shape)
        transformed_point = torch.mm(RT, point)
        print("transformed_point,manual", transformed_point)

        image_size = (2048, 618)

        # Transform the points to screen (pixel) coordinates (u,v)
        screen_points = camera.transform_points_screen(
            point.transpose(0, 1)[:, :3], image_size=image_size
        )
        screen_points = screen_points[:, :2] / screen_points[:, 2].unsqueeze(1)

        print("screen_points pytorch3d", screen_points)

        screen_points = camera.transform_points(point.transpose(0, 1)[:, :3])
        screen_points = screen_points[:, :2] / screen_points[:, 2].unsqueeze(1)

        print("screen_points pytorch3d3", screen_points)

        device = camera.device
        camera = PerspectiveCameras(
            focal_length=camera.focal_length / 4,
            principal_point=camera.principal_point,
            R=camera.R,
            T=camera.T,
        ).to(device)
        ...
        # points_image = camera.get_projection_transform().transform_points(screen_points.unsqueeze(0)).squeeze(0)
        # print("points_image", points_image)

        # # Normalize the projected points
        # points_image = points_image[:, :2] / points_image[:, 2:3]
        # print("points_image, norm", points_image)

        exit()

    if False:
        # Example usage
        focal_length = torch.tensor([[1000.0, 1000.0]], dtype=torch.float32)
        principal_point = torch.tensor([[400.0, 400.0]], dtype=torch.float32)
        R = torch.eye(3, dtype=torch.float32)  # Identity rotation matrix
        T = torch.zeros(3, dtype=torch.float32)  # Zero translation vector
        cameras = PerspectiveCameras(  # Create a PerspectiveCameras object
        ...
        points_2d = points_to_uv(points_3d, cameras)
    ...
    pc_condition = pcpm.point_cloud_to_tensor(pc, normalize=True, scale=True)

    
    if True:
        #test various projections
        gt =  pc_condition.cpu()
        fname=    f"/home/palakons/from_scratch/depth_image_method_1.png"
        
        save_projected_image(gt, fname, point_size=1,
            camera=camera,
            image_rgb=image_rgb.detach().cpu(), i=0,title="method1")
        

        fname=    f"/home/palakons/from_scratch/depth_image_method_2.png"
        
        save_projected_image(gt, fname, point_size=1,
            camera=camera,
            image_rgb=image_rgb.detach().cpu(), i=0,title="method2")

        fname=    f"/home/palakons/from_scratch/depth_image_method_3.png"
        
        save_projected_image(gt, fname, point_size=1,
            camera=camera,
            image_rgb=image_rgb.detach().cpu(), i=0,title="method3")
        
        tx_points_1 = points_to_uv(torch.Tensor(gt[0]).to(camera[0].device), camera[0])
        tx_points_2 = points_to_uv2(torch.Tensor(gt[0]).to(camera[0].device), camera[0])
        tx_points_3 = points_to_uv3(torch.Tensor(gt[0]).to(camera[0].device), camera[0])
        error2_1 = torch.norm(tx_points_2[0] - tx_points_1[0], dim=1)
        error3_1 = torch.norm(tx_points_3[0] - tx_points_1[0], dim=1)
        print("error2_1", error2_1.shape, error2_1)
        point_data = [("code from yesterday",tx_points_1[0],tx_points_1[1],None),("code from yesterday, subtracting the T",tx_points_3[0],tx_points_3[1],None),("use RT as one matrix, the dataset gives 'reverse projection', code: torch.mm(RT.T, points)",tx_points_2[0],tx_points_2[1],None)]
        fname=    f"/home/palakons/from_scratch/depth_image_methods.png"
        save_bulk_projected_image( fname, point_data, point_size=1,  image_rgb=image_rgb[0].detach().cpu(),title="",limit_view=False)

        point_data = [("code from yesterday",tx_points_1[0],tx_points_1[1],error2_1),("code from yesterday, subtracting the T",tx_points_3[0],tx_points_3[1],error3_1)]
        fname=    f"/home/palakons/from_scratch/depth_image_methods-error.png"
        save_bulk_projected_image( fname, point_data, point_size=1,  image_rgb=image_rgb[0].detach().cpu(),title="Error (pixels) from each methods แนทยฟพำก ะน torch.mm(RT.T, points)",limit_view=False)
        exit()


    loss_list = []
    for factor in np.arange(1, 1.2, 0.1):
        loss = plot_image_depth_projected(
            pc_condition.cpu(),
            cfg,
            f"/home/palakons/from_scratch/depth_image_all_{factor:.2f}.png",
            0.1,
            camera=camera,
            image_rgb=image_rgb.detach().cpu(),
            # depth_image=mask[:1].detach().cpu( ) if mask is not None else None,
            depth_image=depths.detach().cpu(),
            factor=factor,
        )
        loss_list.append({"factor": factor, "loss": loss})

    # plot the loss in scatter
    loss_list = pd.DataFrame(loss_list)
    plt.scatter(loss_list.factor, loss_list.loss)
    plt.xlabel("factor")
    plt.ylabel("loss")
    plt.title("loss vs factor")
    plt.savefig(f"/home/palakons/from_scratch/loss_vs_factor.png")

    # for i in range(len(pc)):
    #     pc_condition = pcpm.point_cloud_to_tensor(pc[i:i+1], normalize=True, scale=True)

    #     print("index", idx[i])
    #     print("npoints", npoints[i])
    #     print("camera[0].focal_length", camera[i].focal_length)
    #     print("camera[0].principal_point", camera[i].principal_point)
    #     print("camera[0].R", camera[i].R)
    #     print("camera[0].T", camera[i].T)

    #     plot_image_depth_projected(
    #         pc_condition.cpu(),
    #         cfg,
    #         f"/home/palakons/from_scratch/depth_image_{i}.png",
    #         0.1,
    #         camera=camera[i],
    #         image_rgb=image_rgb[i:i+1].detach().cpu(),
    #         # depth_image=mask[:1].detach().cpu( ) if mask is not None else None,
    #         depth_image=depths[i:i+1].detach().cpu(),
    #     )
