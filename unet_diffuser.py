...
def plot_xy_timestep(
    ...
    for t in range(len(coords)):
        ...
        for i in range(n_current):
            ...
            ax.scatter(sample_coords[:, 0], sample_coords[:, 1], label=f'S{i}: {cd_val.item():.1f}', s=10, alpha=0.6,color ='red')
            # plot gt in blue
            ax.scatter(gt[i, :, 0], gt[i, :, 1], color='blue', label='GT', s=10, alpha=0.4)
...
def train_ptv3_ddpm(config, checkpoint_dir, tb_dir, plot_dir):
    ...
    if config["denoiser_model"] == "simple_unet":
        ...
        model = PTv3Dnsr(
            n_in_channels=3, # We only input XYZ coordinates as features, no additional channels, so n_in_channels=3
        ...
        raise ValueError(f"Unsupported denoiser_model: {config['denoiser_model']}, choose from ['simple_unet', 'ptv3']")
    ...
    for epoch in epoch_bar:
        ...
        for batch in tqdm(train_dataloader, desc="Train Batches", leave=False):
            ...
            normalized_pointcloud_b3n = normalized_pointcloud_bn3.transpose(1, 2).contiguous()
            ...
            if config["debug_mode"] in [1, 2, 3, 5]:
                t_val = max(0, ddpm_scheduler.config.num_train_timesteps // 10)
                t = torch.full((B,), t_val, device=config["device"]).long()
            else:
                t = torch.randint(0, ddpm_scheduler.config.num_train_timesteps, (B,), device=config["device"]).long()
            ...
            if config["lambda_cd"] > 0 or config["lambda_ot"] > 0 or (epoch + 1) % config["plot_every"] == 0:
                ...
                if (epoch + 1) % config["plot_every"] == 0:
                    cd, _ = visualize_xyz_pip(
                        ...
                        original_xyz=G_bn3[0].detach().cpu()*data_std[:3].cpu() + data_mean[:3].cpu(),
                        reconstructed_xyz=P_bn3[0].detach().cpu()*data_std[:3].cpu() + data_mean[:3].cpu(),
        ...
        if (epoch + 1) % config["plot_every"] == 0 or is_best:
                ...
                if param.grad is not None:
                    ...
                    if "head" in name or "ptv3.enc.0" in name:
                        try:
                            ...
                            print(f"Error logging gradient histogram for {name}: {e}", param.grad, param.grad.shape)
            ...
            with torch.no_grad():
                ...
                sample = torch.randn(B, 3, config["num_points"]).to(config["device"])
                ddpm_scheduler.set_timesteps(config["num_inference_steps"], device=config["device"]) #device here is important 
                avg_cd = None
                for i, t in enumerate(ddpm_scheduler.timesteps):
                    t_batch = torch.full((B,), t, dtype=torch.long, device=ddpm_scheduler.timesteps.device)
                    # Predict noise
                    ...
                    pred_x0_xyz_bn3 = normalized_pred_x0_bn3 * data_std[:3] + data_mean[:3]
                    step_cd, _ = pt3d_chamfer_distance(pred_x0_xyz_bn3, pointcloud_bn3, batch_reduction=None)
                    step_cd_np = step_cd.detach().cpu().numpy()
                    all_step_cds.append(step_cd_np)
                    all_pred_xyz_bn3.append(pred_x0_xyz_bn3.detach().cpu().numpy())
                    all_normalized_pred_x0_bn3.append(normalized_pred_x0_bn3.detach().cpu().numpy())
                    sample = prev_sample
                ...
                pred_output = model(normalized_pointcloud_b3n, t, **model_kwargs)
                try:
                    ...
                    print(f"Error logging predicted noise histogram: {e}",pred_output.shape, "min", min(pred_output[:, 0, :]), "max", max(pred_output[:, 0, :]))
