...
class MANDataset(Dataset):
    ...
    def load_data(self, trucksc, frame_token):
        ...
        radar_data = torch.tensor( np.array(cloud.points))
...
def calib_to_camera_base_MAN(cam_calib, cam_pose, radar_calib, radar_pose, image_size_hw: tuple, offset: int = 0, image_size: int = 618, step=4):
    ...
    # print('shaoe T_radar2cam[3, :3]',T_radar2cam[3, :3].shape)
    # print("aaa",(T_radar2cam[3, :3].T).unsqueeze(0).shape)
    # print("aaa",(T_radar2cam[3, :3]).unsqueeze(0).shape)
    # T = (T_radar2cam[3, :3].T).unsqueeze(0)   # (1, 3)
    T = T_radar2cam[3, :3].unsqueeze(0)   # (1, 3)
...
def train(
    ...
    if local_rank == 0:
        os.makedirs(os.path.dirname(checkpoint_fname), exist_ok=True)

        print("rank 0:checkpoint to be saved at", checkpoint_fname)
    ...
    if start_epoch == cfg.run.max_steps :  # no training, checkpoint available
        if local_rank ==0:
            print("start_epoch == cfg.run.max_steps: why runnigng???",start_epoch , cfg.run.max_steps)
    else:
        if local_rank ==0:
        ...
        for epoch in tqdm_range:
            ...
            # )
            batch_train_losses = train_val_one_epoch(
                dataloader_train,  model, optimizer, scheduler, cfg, criterion, device, pcpm, scaler, train_or_val="train"
            )
            batch_train_losses_ave= sum(batch_train_losses) / len(batch_train_losses)
            batch_train_losses_count = len(batch_train_losses)

            dist.all_reduce(batch_train_losses_ave, op=dist.ReduceOp.SUM)
            dist.all_reduce(batch_train_losses_count, op=dist.ReduceOp.SUM)
            
            global_avg_train_loss = batch_train_losses_ave.item() / batch_train_losses_count.item()
            tqdm_range.set_description(f"loss: {global_avg_train_loss:.4f}")

            if local_rank == 0: 
                batch_val_losses= train_val_one_epoch(
                    dataloader_val,  model, optimizer, scheduler, cfg, criterion, device, pcpm, scaler, train_or_val="train"
                )
                ...
                new_train_losses.append(global_avg_train_loss)
                new_val_losses.append(val_losses)


                train_loss_emas = update_loss_emas(
                    loss_ema_factors, train_loss_emas, global_avg_train_loss, epoch, writer=writer, name="train")
            ...
            if (epoch + 1) % cfg.run.vis_freq == 0 and local_rank == 0:
            ...
            if (epoch + 1) % cfg.run.checkpoint_freq == 0 and local_rank == 0:
...
def main(cfg: ProjectConfig):
    ...
    log_dir = obj_list[0]
    print("rank",local_rank,"log_dir", log_dir,"from rank", local_rank)

    if local_rank == 0:
        writer = SummaryWriter(log_dir=log_dir)
        print("rank 0: tensorboard log at", log_dir)
    else:
        writer = None
        print("rank", local_rank, "no tensorboard log")

    # device = "cuda" if torch.cuda.is_available() else "cpu"
    print("rank",local_rank, "device from DDP", device)
    ...
    dataloader_train = DataLoader(
        ...
        batch_size=cfg.dataloader.batch_size,
        num_workers=cfg.dataloader.num_workers,
        sampler=DistributedSampler(dataloader_train.dataset),
        collate_fn=custom_collate_fn_man,
    )
    ...
    # print("train_frame_tokens", train_frame_tokens) # same for all ranks
    # print("val_frame_tokens", val_frame_tokens)
    # print("test_frame_tokens", test_frame_tokens)
    exit()

    dist.barrier()  # Ensure all processes wait until dir creation is complete.
