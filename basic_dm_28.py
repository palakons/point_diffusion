...
def train_one_epoch(dataloader, model, optimizer, scheduler, criterion, device):
    ...
    for batch in dataloader:
        ...
        # expand dim 2 (last dim) from 3 to 4 with zero (one mroe channel)
...
def sample(
    ...
    for i, t in enumerate(tqdm(scheduler.timesteps.to(device), desc="Sampling")):
        ...
        # print("x", x.shape)
        # exit()
        x_4 = torch.cat([x, torch.zeros_like(x[:, :, :1])], dim=-1)
        # print("x", x.shape, "timesteps", timesteps.shape)
...
def get_model(args, device="cpu"):
    ...
    if args.model == "mlp3d":
        ...
        return PVCNNDiffusionModel3D(data_dim=3+1,          point_cloud_model_embed_dim=args.hidden_dim, point_cloud_model="pvcnn",      dropout=0.1,            width_multiplier=1,            voxel_resolution_multiplier=1,).to(device)
    else:
