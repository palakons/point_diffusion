...
class MANDataset(Dataset):
    def __init__(
        ...
        if len(self.data_bank) < self.M:
            print(f"Warning: only {len(self.data_bank)} samples found in scene {self.scene_id}")
    ...
    def load_data(self,trucksc,frame_token):
        ...
        #1) load depth
        depth_image_path = os.path.join(self.data_root, "depth_images", cam['filename'].replace('.jpg', f'_{self.depth_model}.png'))
        ...
        #print calibration
        cam_calib = trucksc.get('calibrated_sensor', cam['calibrated_sensor_token'])
        ...
        image_coord = cam_calib_obj.transform_points(torch.tensor(world_points))
        # torch.tensor(world_points, dtype=torch.float32))  # N x3
        ...
        with open(self.calibration_dir + f"/{idx:06d}.json") as f:
            ...
            for c in calibrations["sensors"]:
                if c["sensor_uid"] == "camera_front":
                    new_camera_base = calib_to_camera_base_astyx(
...
def calib_to_camera_base_MAN(cam_calib, radar_calib, image_size_hw: tuple, offset: int = 0, image_size: int = 618):
    ...
    world_to_radar_RT[:3, :3] = quaternion_to_matrix(torch.tensor([radar_calib['rotation']]))[0]
    ...
    world_to_cam_RT[:3, :3] = quaternion_to_matrix(torch.tensor([cam_calib['rotation']]))[0]
...
def calib_to_camera_base_astyx(calibration_data, image_size_hw: tuple, offset: int = 0, image_size: int = 618):
...
def get_dataset(cfg: ProjectConfig, device="cpu"):
    ...
    if cfg.dataset.type == "astyx":
        ...
        dataloader_train, dataloader_val, _ = get_mandataset(cfg)
    else:
        raise ValueError(
