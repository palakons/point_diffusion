...
def save_checkpoint(model, optimizer, cfg, cd_list,  # (epoch, cd)
                    loss_list, checkpoint_fname, db_fname):
...
def train(
    ...
    assert pcpm is not None, "pcpm must be provided"
    assert len(
        prev_losses) == start_epoch, f"prev_cds and start_epoch should have same length"
    model.train()
    ...
    if start_epoch == cfg.run.max_steps:  # no training, checkpoint available
        ...
        for epoch in tqdm_range:
            ...
            if (epoch + 1) % cfg.run.checkpoint_freq == 0:
                ...
                save_checkpoint(
                    model, optimizer, cfg,  list(prev_cds) + list(new_cds), list(
                        prev_losses) + list(new_losses),
...
def main(cfg: ProjectConfig):
    ...
    if current_cp_fname is not None:
        ...
        if "cds" in current_cp:
            ...
            assert len(
                prev_losses) == start_epoch,            f"prev_losses length {len(prev_losses)} must be equal to start_epoch {start_epoch}"
    else:
