...
def folow_SDK_conversion(trucksc,first_frame_token,test_points,camera: PerspectiveCameras, image_size_hw: tuple, offset: int = 0, image_size: int = 618):
    ...
    T_radar2ego     = to_homogeneous(quat2mat(radar_calib['rotation']), torch.tensor(radar_calib['translation']))
    T_ego2global_r  = to_homogeneous(quat2mat(radar_pose['rotation']), torch.tensor(radar_pose['translation']))
    T_global2ego_c  = to_homogeneous(quat2mat(cam_pose['rotation']), torch.tensor(cam_pose['translation']))
    T_ego2cam       = to_homogeneous(quat2mat(cam_calib['rotation']), torch.tensor(cam_calib['translation']))
    ...
    test_points_homo = torch.cat((test_points, torch.ones(test_points.shape[0], 1)), dim=1)

    test_points_lib = pc_translate(pc_rotate(test_points.T, quat2mat(radar_calib['rotation'])), torch.tensor(radar_calib['translation'])).T
    
    camera_1=calib_to_camera_base_MAN(cam_calib,cam_pose, radar_calib, radar_pose,image_size_hw, offset, image_size, step=1)
    # print("R from cam", camera_1.R)
    # print("T from cam", camera_1.T)
    # print("RT",T_radar2ego.T)
    print("MSE camera_1", torch.mean((camera_1.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
    ...
    print("MSE camera_15", torch.mean((camera_15.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))

    

    test_points_right_se3 = test_points_homo @ T_radar2ego.T
    print("MSE inv_SE3_1", torch.mean((test_points_right_se3[:,:3]-test_points_lib)**2))

    test_points_lib = pc_translate(pc_rotate(test_points_lib.T, quat2mat(radar_pose['rotation'])), torch.tensor(radar_pose['translation'])).T
    
    camera_2=calib_to_camera_base_MAN(cam_calib,cam_pose, radar_calib, radar_pose,image_size_hw, offset, image_size ,step=2)
    print("MSE camera_2", torch.mean((camera_2.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
    ...
    print("MSE camera_25", torch.mean((camera_25.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
   

    test_points_right_se3 = test_points_homo @ T_radar2ego.T @ T_ego2global_r.T
    print("MSE inv_SE3_2", torch.mean((test_points_right_se3[:,:3]-test_points_lib)**2))

    test_points_lib = pc_rotate(pc_translate(test_points_lib.T, -torch.tensor(cam_pose['translation'])), quat2mat(cam_pose['rotation']).T).T
    
    camera_3=calib_to_camera_base_MAN(cam_calib,cam_pose, radar_calib, radar_pose,image_size_hw, offset, image_size, step=3)
    print("MSE camera_3", torch.mean((camera_3.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
    ...
    print("MSE camera_35", torch.mean((camera_35.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
   

    test_points_right_se3 = test_points_homo @  T_radar2ego.T @ T_ego2global_r.T @ T_global2ego_c_inv.T
    print("MSE inv_SE3_3", torch.mean((test_points_right_se3[:,:3]-test_points_lib)**2))

    test_points_lib = pc_rotate(pc_translate(test_points_lib.T, -torch.tensor(cam_calib['translation'])), quat2mat(cam_calib['rotation']).T).T
    
    camera_4=calib_to_camera_base_MAN(cam_calib,cam_pose, radar_calib, radar_pose,image_size_hw, offset, image_size, step=4)
    print("MSE camera_4", torch.mean((camera_4.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
    ...
    print("MSE camera_45", torch.mean((camera_45.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
    ...
    print("MSE inv_SE3", torch.mean((test_points_right_se3[:,:3]-test_points_lib)**2))
    print("MSE camera org", torch.mean((camera.get_world_to_view_transform().transform_points(test_points)-test_points_lib)**2))
...
def calib_to_camera_base_MAN(cam_calib,cam_pose, radar_calib, radar_pose,image_size_hw: tuple, offset: int = 0, image_size: int = 618,step=4):
    ...
    # === Build individual transforms as 4x4 homogeneous matrices ===
    T_radar2ego     = to_homogeneous(quat2mat(radar_calib['rotation']), torch.tensor(radar_calib['translation'])) if step>=1  else torch.eye(4)
    T_ego2global_r  = to_homogeneous(quat2mat(radar_pose['rotation']), torch.tensor(radar_pose['translation'])) if step>=2 else torch.eye(4)
    T_global2ego_c  = to_homogeneous(quat2mat(cam_pose['rotation']), torch.tensor(cam_pose['translation'])) if step>=3 else torch.eye(4)
    T_ego2cam       = to_homogeneous(quat2mat(cam_calib['rotation']), torch.tensor(cam_calib['translation'])) if step>=4 else torch.eye(4)
    ...
    if False:
        ...
        test_points_right =  ( test_points_right @ torch.linalg.inv(T_global2ego_c).T )
        print("test_points_lib",test_points_lib)
        print("test_points_right",test_points_right)
        print("diff right",test_points_right[:,:3]-test_points_lib)
        ...
        print("test_points_right_se3",test_points_right_se3)
        print("diff right",test_points_right_se3[:,:3]-test_points_lib)
        print("MSE", torch.mean((test_points_right_se3[:,:3]-test_points_lib)**2))
...
def train(
    ...
    if start_epoch == cfg.run.max_steps and False:  # no training, checkpoint available
        ...
        plot_ext = "pdf"
        trucksc = TruckScenes(    'v1.0-mini', '/data/palakons/new_dataset/MAN/mini/man-truckscenes', True)
                ...
                for batch in dataloader_train:
                    ...
                    for i in range(len(pc)):  # each data point in batch
                        ...
                        if epoch ==0: #save projected image from SDK
                            ...
                            plt.close()
                            print("npoints,            npoints_filtered  first_frame_token", npoints[i],            npoints_filtered[i],first_frame_token)

                            
                            print("pc_condition shape", pc_condition.shape) #pc_condition shape torch.Size([1, 560, 3])
                        
                            folow_SDK_conversion(trucksc,first_frame_token,pc_condition[0],camera[i],[dataloader_train.dataset.dataset.original_image_size[0]] *2, dataloader_train.dataset.dataset.square_image_offset, dataloader_train.dataset.dataset.img_size)
