...
def plot_sample(gt, xts, x0s, steps, args, epoch, fname):
    '''
    gt: ground truth point cloud (not unnormalized)
    ...
    gt = gt[0].cpu().numpy()
    min_gt = gt.min(axis=0)
    max_gt = gt.max(axis=0)
...
def train(
    ...
    for epoch in tqdm_range:
        ...
        if not args.no_tensorboard and epoch % args.visualize_freq == 0:
            ...
            plot_sample(gt_pc, xts, x0s, steps, args, epoch, fname=None)
