...
def plot_multi_gt(gts,  args,  fname):
    if fname is None:
        fname = f"outputs/gts_M{args.M}" + \
            args.run_name.replace("/", "_")+".png"
    gts = gts.cpu().numpy()
    ...
    fig = plt.figure(figsize=(10*args.M, 10))
    for i, gt in enumerate(gts):
        ax = fig.add_subplot(1, len(gts), i+1, projection='3d')
...
def plot_sample_multi_gt(gts, xts, x0s, steps, args, epoch, fname):
    '''
    ...
    '''
    plot_multi_gt(gts,  args,  fname=None)
    if fname is None:
        fname = f"outputs/sample_ep{epoch:09d}_" + \
            args.run_name.replace("/", "_")+".png"
