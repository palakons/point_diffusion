...
class SimpleDenoiser(nn.Module):
    ...
    def forward(self, x_t, t):
        ...
        t_embed = t_embed.unsqueeze(1).expand(-1, x_t.shape[1], -1)
        # print("x_t", x_t.shape)  # (B, N, D+id_emb_dim)
        # print("t_embed", t_embed.shape)  # (B, N, 128)
...
def main():
    ...
    x_0_data = get_man_data(N, radar_ch).to(device).float()
    print("x_0_data", x_0_data.shape)  # (1, N, 3)
    x_0_id = get_sinusoidal_time_embedding(
        torch.tensor(range(N)).to(device), id_emb_dim).to(device).float()
    print("x_0_id", x_0_id.shape)  # (N, id_emb_dim)
    x_0 = torch.cat([x_0_data, x_0_id.unsqueeze(0)], dim=-1)
    print("x_0", x_0.shape)  # (1, N, 3+4)

    data_mean, data_std = x_0.mean(dim=[0, 1]), x_0.std(dim=[0, 1])
    print("data_mean", data_mean)
    print("data_std", data_std)
    x_0_id_normed = ((x_0_id - data_mean[3:]) / data_std[3:]).to(device)
    x_0_data_normed = ((x_0_data - data_mean[:3]) / data_std[:3]).to(device)
    ...
    model = SimpleDenoiser(input_dim=D+id_emb_dim, output_dim=D,
    ...
    for i_epoch, epoch in enumerate(tt):
        ...
        noise = torch.randn_like(x_0_data_normed)  # (B, N, D)
        x_t = scheduler.add_noise(x_0_data_normed, noise, t)  # (B, N, D)
        x_t = torch.cat([x_t, x_0_id_normed.unsqueeze(0)],
                        dim=-1)  # (B, N, D+id_emb_dim)
        model.train()
        # print("x_t", x_t.shape)  # (B, N, D+id_emb_dim)
        # print("t", t.shape)  # (B,)
        pred_noise = model(x_t, t)  # (B, N, D)
        ...
        if i_epoch % vis_freq == 0:
            xts_tensor_list, steps, cd_loss = sample(
                model.eval(), scheduler, T, B, N, D, x_0_data_normed, device, data_mean=data_mean, data_std=data_std, x_0_id_normed=x_0_id_normed)
            ...
            save_sample_json(path, epoch, x_0_data_normed, xts_tensor_list,
                             steps,  cd=cd_loss, data_mean=data_mean[:3], data_std=data_std[:3], config={
                                 "B": B,
                                 "N": N,
                                 "D": D,
                                 "T": T,
                                 ...
                                 "lr": lr,
                                 "radar_ch": radar_ch, "id_emb_dim": id_emb_dim
                             })
...
def sample(model, scheduler, T, B, N, D, x_0, device, data_mean, data_std, x_0_id_normed):
    """
    Sample from the model using the DDPM scheduler.
    Args:
        ...
        D: int Dimension of the point cloud.
        x_0: N x 3 tensor of ground truth point cloud.
        device: Device to use (CPU or GPU).
        data_mean: 3 Mean used for normalization.
        data_std: 3 Std used for normalization.
        x_0_id_normed: N x dim Normalized id embedding."""
    ...
    with torch.no_grad():
        for t in scheduler.timesteps:
            ...
            # (B, N, D+id_emb_dim)
            x_t = torch.cat(
                [x_t.to(device), x_0_id_normed.unsqueeze(0).to(device)], dim=-1)
            noise_pred = model(x_t.to(device), t_tensor)  # (B, N, D)
            x_t = scheduler.step(
                noise_pred.cpu(), t_tensor.cpu(), x_t[:, :, :3].cpu()).prev_sample
            xts_tensor_list.append(x_t[0, :, :3].cpu())

    sampled = x_t[:, :, :3].to(device)
    data_std = data_std[:3].to(device)
    data_mean = data_mean[:3].to(device)
