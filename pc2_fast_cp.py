...
def train(
    ...
    checkpoint_fname = (
        f"{CHECKPOINT_DIR}/cp_dm_{datetime.now().strftime(f'%Y-%m-%d-%H-%M-%S')}-{cfg.run.name.replace('/', '_') }_{os.uname().nodename}.pth"
    )
    # mkdir if not exist
    ...
    if start_epoch == cfg.run.max_steps: # no training, checkpoint available
        ...
        for epoch in tqdm_range:
            ...
            if (epoch + 1) % cfg.run.checkpoint_freq == 0:
                ...
                save_checkpoint(checkpoint,checkpoint_fname.replace(".pth", f"_{epoch}.pth"), f"{CHECKPOINT_DIR}/{CHECKPOINT_DB_FILE}")
        ...
        save_checkpoint(checkpoint,checkpoint_fname.replace(".pth", f"_final.pth"), f"{CHECKPOINT_DIR}/{CHECKPOINT_DB_FILE}")
...
def match_args(cfg1: CO3DConfig, cfg2: CO3DConfig):
    ...
    if cfg1.model.beta_schedule != cfg2.model.beta_schedule:
        print("beta_schedule mismatch", cfg1.model.beta_schedule, cfg2.model.beta_schedule)
    ...
    if cfg1.dataloader.batch_size != cfg2.dataloader.batch_size:
        print("batch_size mismatch", cfg1.dataloader.batch_size, cfg2.dataloader.batch_size)
    if cfg1.dataloader.num_scenes != cfg2.dataloader.num_scenes:
        print("num_scenes mismatch", cfg1.dataloader.num_scenes, cfg2.dataloader.num_scenes)
    ...
    if cfg1.optimizer.weight_decay != cfg2.optimizer.weight_decay:
        print("weight_decay mismatch", cfg1.optimizer.weight_decay, cfg2.optimizer.weight_decay)
    if cfg1.optimizer.kwargs.betas[0] != cfg2.optimizer.kwargs.betas[0]:
        print("beta1 mismatch", cfg1.optimizer.kwargs.betas[0], cfg2.optimizer.kwargs.betas[0])
    if cfg1.optimizer.kwargs.betas[1] != cfg2.optimizer.kwargs.betas[1]:
        print("beta2 mismatch", cfg1.optimizer.kwargs.betas[1], cfg2.optimizer.kwargs.betas[1])
        ...
        try:
            ...
            if match_args(checkpoint["args"], cfg):
                if (
                    ...
                    print("epoch in config",checkpoint["args"].run.max_steps ," is less than max epoch in checkpoint", cfg.run.max_steps,"or max epoch in checkpoint is less than max epoch in checkpoint",  max_epoch)
        except:
            print("error", fname)
            continue
...
def main(cfg: ProjectConfig):
    ...
    if cfg.checkpoint.resume_training:
        if os.path.exists(f"{CHECKPOINT_DIR}/{CHECKPOINT_DB_FILE}"):
            current_cp_fname = get_checkpoint_fname_json(cfg, CHECKPOINT_DB_FILE,CHECKPOINT_DIR)
