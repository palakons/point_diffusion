...
def main():
    ...
    ddpm, lr_scheduler, optimizer, global_step, epoch = train_dit_diffusion(
        dataset_point,
        config,
    ...
    # Save DDPM model
    # todo: where to save
    model_save_path = f"{checkpoint_dir}/ddpm_model_dit_{runid}.pt"
    torch.save(
        {
            "model_state_dict": ddpm.model.state_dict(),
            "depth_tokenizer_state_dict": ddpm.depth_tokenizer.state_dict(),
    ...
    print(f"Saved DiTDDPM model to {model_save_path}")
    ...
    if "uvz" in predicted_uvz:
        ...
        for t in outputs:
            dataset_point.visualize_uvz_comparison(
                title=runid + "_dit_trained",
                frame_token=sample_data["frame_token"][-5:] + f"_{runid}_dit_at{t}",
