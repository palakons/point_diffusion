...
SchedulerClass = Union[DDPMScheduler, DDIMScheduler,
...
class PointCloudProjectionModel(ModelMixin):
    ...
    def __init__(
        ...
        formatter = logging.Formatter(
            f"%(asctime)s,%(name)s,%(levelname)s,{run_name},%(message)s")
    ...
    def get_global_conditioning(self, image_rgb: Tensor):
        ...
        global_conditioning = torch.cat(
            global_conditioning, dim=1)  # (B, D_cond)
    ...
    def get_local_conditioning(self, image_rgb: Tensor, mask: Tensor):
        ...
        local_conditioning = torch.cat(
            local_conditioning, dim=1)  # (B, D_cond, H, W)
    ...
    def surface_projection(
        ...
        rasterizer = PointsRasterizer(
            cameras=camera, raster_settings=self.raster_settings)
    ...
    def point_cloud_to_tensor(self, pc: Pointclouds, /, normalize: bool = False, scale: bool = False):
        ...
        if self.predict_color and pc.features_padded() is not None:
            colors = self.normalize(
                pc.features_padded()) if normalize else pc.features_padded()
    ...
    def tensor_to_point_cloud(self, x: Tensor, /, denormalize: bool = False, unscale: bool = False):
        ...
        if self.predict_color:
            colors = self.denormalize(
                x[:, :, 3:]) if denormalize else x[:, :, 3:]
    ...
    def get_input_with_conditioning(
        ...
        if self.use_local_conditioning:
            ...
            local_features = self.get_local_conditioning(
                image_rgb=image_rgb, mask=mask)
            if local_features.shape[-2:] != image_rgb.shape[-2:]:
                raise ValueError(
                    f'{local_features.shape=} and {image_rgb.shape=}')

            self.logger.info(
                f"{os.getpid()}, get_input_with_conditioning/1_get_local_conditioning, {time.time()-st_time}")
            st_time = time.time()
            ...
            self.logger.info(
                f"{os.getpid()}, get_input_with_conditioning/2_surface_projection, {time.time()-st_time}")
            st_time = time.time()
        # Global conditioning
        if self.use_global_conditioning:

            # Get and repeat global features
            global_features = self.get_global_conditioning(
                image_rgb=image_rgb)  # (B, D_global)
            global_features = global_features.unsqueeze(
                1).expand(-1, N, -1)  # (B, D_global, N)

            x_t_input.append(global_features)

        self.logger.info(
            f"{os.getpid()}, get_input_with_conditioning/3_use_global_conditioning, {time.time()-st_time}")
        st_time = time.time()
        # Concatenate together all the pointwise features
        x_t_input = torch.cat(x_t_input, dim=2)  # (B, N, D)

        self.logger.info(
            f"{os.getpid()}, get_input_with_conditioning/4_torch.cat, {time.time()-st_time}")
        return x_t_input
