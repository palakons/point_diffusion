...
MODEL_PATHS = {
    'vit_base_patch16_224_mae': '/data/palakons/models/vit_base_patch16_224_mae.pth',
    'vit_small_patch16_224_msn': '/data/palakons/models/vit_small_patch16_224_msn.pth.tar',
    'vit_large_patch7_224_msn': '/data/palakons/models/vit_large_patch7_224_msn.pth.tar',
}
...
class FeatureModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if  finetune_vits and vits_checkpoint is not None:
            checkpoint = vits_checkpoint
            print(f"Loading checkpoint from vits_checkpoint")
        else:
            checkpoint = torch.load(MODEL_PATHS[model_name])
            print(f"Loading checkpoint from MODEL_PATHS[model_name]", MODEL_PATHS[model_name])   
        ...
        state_dict['pos_embed'] = resize_pos_embed(
            state_dict['pos_embed'], self.model.pos_embed)
        self.model.load_state_dict(state_dict)
        
        if not finetune_vits:
            self.model.eval() #default case 
    ...
    def denormalize(self, img: Tensor):
        img = TVF.normalize(
            img, mean=[-m/s for m, s in zip(self.mean, self.std)], std=[1/s for s in self.std])
    ...
    def forward(
        ...
        if return_type in {'features', 'all'}:
            ...
            output_feats: Tensor = feats[:, 1:, :].reshape(
                B, HW_down, HW_down, D).permute(0, 3, 1, 2)  # (B, D, H_down, W_down)
