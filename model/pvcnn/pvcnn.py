...
class PVCNN2Base(nn.Module):
    def __init__(
        ...
        # print(f"use_att: {use_att}")
        # print(f"dropout: {dropout}")
        # print(f"extra_feature_channels: {extra_feature_channels}")
        # print(f"width_multiplier: {width_multiplier}")
        # print(f"voxel_resolution_multiplier: {voxel_resolution_multiplier}")
        # exit()
        super().__init__()
        ...
        self.global_att = None if not use_att else Attention(
            channels_sa_features, 8, D=1)
        ...
        self.embedf1 = nn.Linear(embed_dim, embed_dim)
        self.embedf1_5 = nn.LeakyReLU(0.1, inplace=True)
        self.embedf2 = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, inputs: torch.Tensor, t: torch.Tensor):
        ...
        # assert non nan in t_emb
        assert not torch.isnan(t_emb).any(), "t_emb has nan"

        xxx = self.embedf1(t_emb)

        assert not torch.isnan(xxx).any(), "xxx has nan"
        xxx = self.embedf1_5(xxx)


        xxx = self.embedf2(xxx)
        # assert, not all elements in xxx are nan
        t_emb = xxx[:, :, None].expand(-1, -1, inputs.shape[-1])
        # print("t_emb3", t_emb[0])
        ...
        for i, sa_blocks in enumerate(self.sa_layers):
            ...
            if i == 0:
                ...
                features, coords, t_emb = sa_blocks(
                    (torch.cat([features, t_emb], dim=1), coords, t_emb))
        ...
        for fp_idx, fp_blocks in enumerate(self.fp_layers):
            features, coords, t_emb = fp_blocks(
                (  # this is a tuple because of nn.Sequential
                    ...
                    torch.cat([features, t_emb], dim=1),
