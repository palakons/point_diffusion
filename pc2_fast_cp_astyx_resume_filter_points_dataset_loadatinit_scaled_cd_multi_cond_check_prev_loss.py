...
def train(
    ...
    print("will export emas to pickle ")
    ...
    if False:
        import pickle
        pkl_fname = "/home/palakons/from_scratch/emas_pkl.pkl"
...
def process_ema_prev_values(cfg: ProjectConfig, writer, ema_factors, epochs, prev_values: dict, name):
    print("process_ema_prev_values", name, "factors", ema_factors)
    ...
    for scene_id, losses in tqdm(prev_values.items()):  # {scene_id: [loss]}
        ...
        for alpha in tqdm(ema_factors, desc=f"ema factors for {scene_id}", leave=False):
            ...
            print(f"raw losses factor {alpha}", losses[:10])
            print(f"emas from aggregate factor {alpha}", ema_list[:10])
            ...
            for epoch, ema in tt:
                ...
                try:
                    ...
                    print("error in adding scalars", e)
                    pass
    ...
    for alpha in ema_factors:
        ...
        for epoch, ema in zip(
            ...
            try:
                ...
                print("error in adding scalars", e)
                pass
                # print(f"{name}/average {epoch} {ema} {alpha:.2e}")
    return emas
...
def main(cfg: ProjectConfig):
    ...
    current_cp_fname = None
    # current_cp_fname="cp_dm_2025-04-02-20-21-24-300k_cond_test_size_64-600k_vision05_231263_599999.pth
    current_cp_fname = f"{CHECKPOINT_DIR}/cp_dm_2025-04-02-20-21-24-300k_cond_test_size_64-600k_vision05_231263_599999.pth"
    ...
    if current_cp_fname is not None:
        ...
        if False:
            ...
            print("hey")
            print("keys", current_cp.keys())
            # keys dict_keys(['model', 'optimizer', 'args', 'cds', 'losses'])
            print("cds", current_cp["cds"].keys())
            # keys dict_keys(['epochs', 'data'])
            # cds dict_keys([137, 64, 261, 120])
            # cds dict_keys([137, 64, 261, 120])
            print("len current_cp[cds][epochs]",
                  len(current_cp["cds"]["epochs"]))  # len current_cp[cds][epochs] 956
            print("cds", current_cp["cds"]["data"].keys())

            print("len  current_cp[cds][data][137]",
                  len(current_cp["cds"]["data"][137]))
            print("losses", current_cp["losses"].keys())
            # keys dict_keys(['ave'])
            print("len  current_cp[losses][ave]",
                  len(current_cp["losses"]["ave"]))
            # len  current_cp[cds][data][137] 239
            # len  current_cp[losses][ave] 600000
